...
Architecture (plan §6):
  ...
  player, global, passives, last_action  ─► MLPs ─┐
...
Factored MultiDiscrete action space: [9, 5, 2, 2, 2].
"""
...
class IsaacPolicy(nn.Module):
    ...
    def __init__(self, cfg: PolicyConfig | None = None):
        ...
        self.player_mlp   = _mlp([PLAYER_DIM, 128, 128], activate_final=True)
        self.global_mlp   = _mlp([GLOBAL_DIM, 64, 64], activate_final=True)
        self.passives_mlp = _mlp([PASSIVES_K, 64], activate_final=True)
        self.last_action_mlp = _mlp([len(ACTION_FACTORS), 32], activate_final=True)
        ...
        self.doors_mlp = _mlp([4 * 6, 32], activate_final=True)

        trunk_in = 128 + 64 + 64 + 32 + c.entity_dim + c.proj_dim + c.pickup_dim + gc2 + 32
    ...
    def encode(self, obs: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        x = torch.cat([player, global_, passives, last_act, enemies, projs, pickups, grid, doors], dim=-1)
        return self.trunk(x)
    ...
    def step(
        ...
        logits = [head(new_hidden) for head in self.heads]
